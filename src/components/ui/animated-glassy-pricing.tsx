  ...
  return (
    ...
    >
      ...
      <div className="mb-4 sm:mb-5">
        <h3 className="text-xl xs:text-2xl font-semibold text-[#c61951] tracking-wide">
          {planName}
        </h3>
        <p className="text-xs xs:text-sm text-gray-400 mt-1.5 xs:mt-2 leading-relaxed">
          {description}
        </p>
      </div>
      <div className="mb-4 xs:mb-5 sm:mb-6 flex items-baseline">
        <span className="text-3xl xs:text-4xl sm:text-5xl font-light text-white">
          ${price}
        </span>
      ...
      <ul className="space-y-2 xs:space-y-3 mb-6 xs:mb-7 sm:mb-8 flex-1">
        {features.map((feature, index) => (
          <li
            key={index}
            className="flex items-center gap-2 xs:gap-3 text-xs xs:text-sm text-gray-300"
          >
            <CheckIcon className="w-3.5 h-3.5 xs:w-4 xs:h-4 text-[#c61951] shrink-0" />{" "}
            {feature}
          </li>
        ))}
      </ul>
